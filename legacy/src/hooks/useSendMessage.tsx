...
export default function useSendMessage() {
	const [_, setMessages] = useLocalStorage<MessageData[]>({
		key: "messages",
		defaultValue: [],
	});
	const [openAiModel] = useLocalStorage({
		key: "open-ai-model",
		defaultValue: "",
	});

	const { setIsStreaming, setStreamingMessage, openAiClient, setOpenAiClient } =
		useLlmContext();
	...
	return useCallback(
		async (messages: MessageData[]) => {
			...
			if (!openAiClient) {
				console.info(
					"INFO - OpenAI client is not initialized when sending message; attempting to instantiate",
				);

				const newOpenAiClient = createOpenAiClient();
				if (!newOpenAiClient) {
					console.warn(
						"WARN - failed to create OpenAI client when sending message",
					);
				...
				console.log(
					"INFO - OpenAI client created successfully when sending message",
				);
			}

			if (!actualOpenAiClient) {
				console.error(
					"ERROR - OpenAI client is still undefined after creation attempt",
				);
			...
			for await (const part of stream) {
				...
				fullResponse +=
					part.choices[0]?.delta?.content?.replaceAll("—", " - ") || "";
