...
export default function PortraitUploader({
	...
	const [portraitTab, setPortraitTab] = useState<PortraitType>("comfort");
	...
	const tabOptions: PortraitType[] = nsfw
		? ["nude", "comfort", "travel", "battle"]
		: ["comfort", "travel", "battle"];
	...
	return (
		<Stack gap="xs">
			...
			>
				<TabsList grow>
					{tabOptions.map((tab) => (
						<TabsTab key={`appearance-portrait-tab-${tab}`} value={tab}>
							{tabLabel(tab)}
						</TabsTab>
					))}
				</TabsList>
				...
				{tabOptions.map((tab) => {
					...
					return (
						<TabsPanel key={`appearance-portrait-${tab}`} value={tab} pt="xs">
							<Stack gap="xs">
								<Box
									style={{
										aspectRatio: "9 / 16",
...
function tabLabel(portrait: PortraitType) {
	return portrait.charAt(0).toUpperCase() + portrait.slice(1);
}
