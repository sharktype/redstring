...
export default function Agent(props: AgentProps) {
  const [openAiApiKey, setOpenAiApiKey] = useLocalStorage({ key: "open-ai-api-key", defaultValue: "" });
  const [openAiBaseUrl, setOpenAiBaseUrl] = useLocalStorage({ key: "open-ai-base-url", defaultValue: "" });
  const [openAiModel, setOpenAiModel] = useLocalStorage({ key: "open-ai-model", defaultValue: "" });

  const { openAiClient, setOpenAiClient } = useLlmContext();
  const createOpenAiClient = useCreateOpenAiClient();

  return (
    <Modal title="Agent" opened={props.isOpened} onClose={() => props.close()} centered>
      <Title order={2} mb="md">
        OpenAI Credentials
      </Title>
